...
Greedy decoding is deterministic and the extraction strategies share prompts
(``llm_validated`` re-issues every ``llm`` section prompt before its targeted re-asks; the
router evaluation re-runs the extractor), so caching answers keyed by the exact request
roughly halves the cost of a real-model evaluation and makes a re-run free. Only
``temperature == 0`` requests are cached; sampled requests bypass the cache.
"""
...
from advicedoc.llm import ChatMessage, ChatModel, ChatResponse
