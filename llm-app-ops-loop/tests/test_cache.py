...
from opsloop.cache import CachedChatModel, request_key
from opsloop.llm import ChatMessage, ChatResponse
...
def test_cache_key_covers_request_parameters(tmp_path: Path) -> None:
    ...
    cached.chat([ChatMessage("tool", "hello")], max_tokens=10)  # role is part of the key
...
def test_hf_backend_is_shared_per_model_name() -> None:
    # The regression gate builds an answerer and a judge; with the same model name they must
    # be the same in-process object (two 4 B copies do not fit a 12 GB card).
    from opsloop.llm import build_model

    a = build_model("hf", model_name="D:/models/some-model")
    b = build_model("hf", model_name="D:/models/some-model")
    c = build_model("hf", model_name="D:/models/other-model")
    assert a is b
    assert a is not c and c.name == "hf[D:/models/other-model]"
